        ...
        <div>
          ...
          <div className="flex px-4 space-x-5 mt-3 text-gray-700 py-2.5 border-t border-b-[2px]  w-full">
            ...
            className={cls(
              ...
              <span>Wonder {data?.post._count.wonderings}</span>
            </button>
            <span className="flex space-x-2 items-center text-sm">
              ...
              <span>Comment {data?.post._count.answers}</span>
            </span>
          </div>
        </div>
        <div className="px-4 my-5 space-y-5">
          {data?.post.answers.map(answers => (
            <div key={answers.id} className="flex items-start space-x-3">
            <div className="w-8 h-8 bg-slate-200 rounded-full" />
            <div>
              <span className="text-sm block font-medium text-gray-700">
                {answers.user.name}
              </span>
              <span className="text-xs text-gray-500 block ">{answers.createdAt.toString()}</span>
